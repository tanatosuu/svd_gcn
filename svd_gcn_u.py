...
class SVD_GCN(nn.Module):
	def __init__(self, user_size, item_size, beta=2.0,coef_u=0.7, req_vec=240, latent_size=64, reg=0.01):
	...
	def forward(self,u,p,n,up,un):
		...
		self_loss=torch.log(((final_user*final_user_p).sum(1)-(final_user*final_user_n).sum(1)).sigmoid()).sum()
		regu_term=self.reg*(final_user**2+final_pos**2+final_nega**2+final_user_p**2+final_user_n**2).sum()

		return (-torch.log(out).sum()-self.coef_u*self_loss+regu_term)/batch_size
