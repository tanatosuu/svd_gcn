...
class SVD_GCN(nn.Module):
	def __init__(self, user_size, item_size, beta=2.0,coef_i=0.7, req_vec=60, latent_size=64, reg=0.01):
	...
	def forward(self,u,p,n,pp,pn):
		...
		self_loss=torch.log(((final_pos*final_pos_p).sum(1)-(final_pos*final_pos_n).sum(1)).sigmoid()).sum()
		regu_term=self.reg*(final_user**2+final_pos**2+final_nega**2+final_pos_p**2+final_pos_n**2).sum()

		return (-torch.log(out).sum()-self.coef_i*self_loss+regu_term)/batch_size
